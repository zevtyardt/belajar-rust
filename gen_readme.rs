use std::{fs::{read_dir, read_to_string, File}, path::Path, collections::BTreeMap, io::Write};
...
fn walkdir(dir: &Path, indent: usize, lines: &mut Vec<String>, data: &mut BTreeMap<String, Vec<BTreeMap<String, String>>>) {
    if dir.is_dir() {
        for entry in read_dir(dir).unwrap() {
            ...
            if path.is_dir() && ![".git", "target", "src"].contains(splited.last().unwrap()) {
                ...
                if desc.len() > 0 {
                    ...
                    subdata.insert(String::from("folder"), String::from(
                            format!("[{dirname}](/{dirname})", dirname=dirname.strip_prefix("./").unwrap().display())
                    ));


                    data.get_mut(desc[1])
                        .map(|val| val.push(subdata));
                }
            }
        }
    }
...
fn main() {
    ...
    walkdir(&start, 0, &mut lines, &mut data);
    ...
    for (index, (tanggal, projects)) in data.iter().enumerate() {
        ...
        write!(readme_file, "|{}|{}|{}|{}|\n", index + 1, tanggal, 
            ...
            first_project.get("desc").unwrap()).unwrap();
        for project in projects[1..].iter() {
            write!(readme_file, "|||{}|{}|",
                project.get("folder").unwrap(),
                project.get("desc").unwrap()).unwrap();
        };
    }
